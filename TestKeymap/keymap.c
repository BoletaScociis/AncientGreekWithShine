...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        case KC_ESCTILDE:
            ...
            register_code(KC_TILD); // Change the key to be held here
            } else {
            unregister_code(KC_TILD); // Change the key that was held here, too!
            if (timer_elapsed(KC_ESCTILDE_TIMER) < 200) {
            SEND_STRING("ESC"); // Change the character(s) to be sent on tap here
        }
      }
